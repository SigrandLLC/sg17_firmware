	...
	var sendTask = function(task) {
		...
		var options = {
			...
			"success": function(data, textStatus) {
				wf2Logs.addLog("AJAX task response",
							   $.sprintf("status: %s, url: %s, data: %s",
	...
	this.cmdExecute = function(options) {
		...
		var ajaxOptions = {
			...
			"success": function(data, textStatus) {
				...
				{
					wf2Logs.addLog("AJAX response",
								   $.sprintf("status: %s, url: %s, cmd: %s, data: %s",
					...
					if (ajaxNum == 0) {
						...
						$("#status_ajax").text($.sprintf("%s (%s: %s)",
														 _("loading data"), _("requests"),
														 ajaxNum));
					}
				}
				processResult(data);
			...
			"error": function(XMLHttpRequest, textStatus, errorThrown) {
				if (options.status != false)
				{
					wf2Logs.addLog("AJAX error",
								   $.sprintf("status: %s, url: %s, cmd: %s",
											 textStatus, this.url, this.data));
