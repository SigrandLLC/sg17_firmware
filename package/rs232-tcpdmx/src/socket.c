...
static void set_sock_opt_int(int fd, int opt, int optval, const char *optname, const char *pfx)
{
    ...
    if (rc < 0)
    {
	dferror(EXIT_SUCCESS, errno, "%sCould not set %s socket option", pfx, optname);
	fail();
    }
}
...
static void set_sock_linger(int fd, int timeout, const char *pfx)
{
    ...
    if (rc < 0)
    {
	dferror(EXIT_SUCCESS, errno, "%sCould not set linger", pfx);
	fail();
    }
}
...
void socket_bind(socket_t *s, const char *host, const char *port)
{
    ...
    if (rc != 0)
    {
	dflog(LOG_ERR, "%s(): getaddrinfo(%s, %s, ...): %s",
	      __FUNCTION__, host, port, gai_strerror(rc));
	fail();
    }
    ...
    {
        errno = errno_save;
	dferror(EXIT_SUCCESS, errno, "%s(): Could not bind to %s:%s",
    ...
    if (rc < 0)
    {
	dferror(EXIT_SUCCESS, errno, "%s(): listen(2) failed", __FUNCTION__);
	fail();
    }
...
socket_t *socket_accept(socket_t *s)
{
    ...
    if (newfd < 0)
    {
	dferror(EXIT_SUCCESS, errno, "%s(): accept(2) failed", __FUNCTION__);
	fail();
    }
    ...
    if (rc != 0)
    {
	dflog(LOG_WARNING, "%s(): Connection from unknown place, getnameinfo(3) failed: %s\n",
...
int socket_connect(socket_t *s, const char *host, const char *port)
{
    ...
    if (rp == NULL)	// No address succeeded
    {
	dflog(LOG_ERR, "%s(): Could not connect to %s:%s",
	      __FUNCTION__, host, port);
	return 1;
    }
...
void socket_set_ip_tos(socket_t *s, int ip_tos)
{
    ...
    if (rc < 0)
    {
	dferror(EXIT_SUCCESS, errno, "Could not set IP ToS option");
	fail();
    }
}
