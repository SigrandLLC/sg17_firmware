#!/usr/bin/haserl
	...
	render_list_btns(){
		...
		# edit
		echo "<a href='javascript:popupWin = window.open(\"index.cgi?controller=dhcp_static_edit&item=${item}&popup=1\", \"popup\", \"width=500,height=500,top=0,modal=1,dialog=1,centerscreen=1,scrollbars=0,menubar=0,location=0,toolbar=0,dependent=1,status=0\"); popupWin.focus();'>Edit</a>"
