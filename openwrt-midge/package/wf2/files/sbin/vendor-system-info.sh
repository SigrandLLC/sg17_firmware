...
cp /proc/meminfo $STDIR/meminfo.`date -I`.log
...
for i in $dsl_ifs; do
	if [ -d /sys/class/net/$i/sg_private ]; then
		cp /sys/class/net/$i/sg_private/regs   $STDIR/$i-regs.`date -I `.log
	elif [ -d  /sys/class/net/$i/sg17_private ]; then
		cp /sys/class/net/$i/sg17_private/regs $STDIR/$i-regs.`date -I `.log
	fi
done
