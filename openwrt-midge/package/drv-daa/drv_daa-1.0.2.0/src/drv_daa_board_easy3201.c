...
#include "drv_dxt_io_types.h"
#include "drv_dxt_gpio.h"

extern DAA_FUNCTION_MAP_t        gDaaFctMap[];

int dxtDevHandle, dxtDevIoHandle = 0;
...
{
   DXT_GPIO_Config_t ioCfg;
   int ret;

   /* DuSLIC-xT FXO is on reference board, which is nDevNr = 1 */
   dxtDevHandle = DXT_OpenKernel(1, 0);
   if (dxtDevHandle == IFX_ERROR)
   {
      printk("open failed\n\r");
      return IFX_ERROR;
   }
   ...
   /*
      Reserve GPIO for the following functionalities:
      DXT_IO_DEV_GPIO_2: HOOK control
      DXT_IO_DEV_GPIO_3: Ring detection
      DXT_IO_DEV_GPIO_4: Battery detection
      DXT_IO_DEV_GPIO_5: Polarity detection
      DXT_IO_DEV_GPIO_7: optional APOH detection
      Note: CID DXT_IO_DEV_GPIO_1 currently not supported
   */
   dxtDevIoHandle = DXT_GpioReserve(dxtDevHandle, DXT_IO_DEV_GPIO_2 |
                                                  DXT_IO_DEV_GPIO_3 |
                                                  DXT_IO_DEV_GPIO_4 |
                                                  DXT_IO_DEV_GPIO_5 |
                                                  DXT_IO_DEV_GPIO_7);
   ...
   /* HOOKQ */
   ioCfg.nMode = DXT_GPIO_MODE_OUTPUT;;
   ioCfg.nGpio = DXT_IO_DEV_GPIO_2;
   ret = DXT_GpioConfig(dxtDevIoHandle, &ioCfg);
   if (ret != IFX_SUCCESS)
   {
   ...
   /* RINGQ */
   ioCfg.nMode = DXT_GPIO_MODE_INPUT | DXT_GPIO_MODE_INT | DXT_GPIO_INT_FALLING;
   ioCfg.nGpio = DXT_IO_DEV_GPIO_3;
   ...
   /* BATQ */
   ioCfg.nMode = DXT_GPIO_MODE_INPUT | DXT_GPIO_MODE_INT |
                 DXT_GPIO_INT_RISING | DXT_GPIO_INT_FALLING;
   ioCfg.nGpio = DXT_IO_DEV_GPIO_4;
...
static IFX_return_t daa_0_hookSet (IFX_TAPI_FXO_HOOK_t offhook)
{
   ...
   if (dxtDevIoHandle != 0)
   {
      ret = DXT_GpioSet(dxtDevIoHandle,
                       (offhook == IFX_TAPI_FXO_HOOK_OFFHOOK) ? 0x00 : DXT_IO_DEV_GPIO_2,
                        DXT_IO_DEV_GPIO_2);
   }
   else
   {
...
static IFX_return_t daa_0_hookGet (IFX_TAPI_FXO_HOOK_t *offhook)
{
   ...
   if (dxtDevIoHandle != 0)
   {
      ret = DXT_GpioGet(dxtDevIoHandle, &get, DXT_IO_DEV_GPIO_2);
...
static IFX_return_t daa_0_batGet (IFX_enDis_t *bat)
{
   ...
   if (dxtDevIoHandle != 0)
   {
      ret = DXT_GpioGet(dxtDevIoHandle, &get, DXT_IO_DEV_GPIO_4);
...
static IFX_return_t daa_0_polGet (IFX_enDis_t *pol)
{
   ...
   if (dxtDevIoHandle != 0)
   {
      ret = DXT_GpioGet(dxtDevIoHandle, &get, DXT_IO_DEV_GPIO_5);
...
static IFX_return_t daa_0_apohGet (IFX_enDis_t *apoh)
{
   ...
   if (dxtDevIoHandle != 0)
   {
      ret = DXT_GpioGet(dxtDevIoHandle, &get, DXT_IO_DEV_GPIO_7);
...
/**
   This function is getting called on rmmod of drv_daa in Linux sytems.
   Note that at this time you cannot access other device's GPIOs as
   they aren't yet initialized.
*/
