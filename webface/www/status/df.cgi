...
<table>
<? printTableTitle "Disk information" 6 ?>
...
<? echo "$(df | grep -v Filesystem | sed "s/ \+/ /g" | cut -f 1,2,3,5,6 -d' ')" | \
    while read dev size used avail fs; do 
        type=$(getFsType $dev)
        echo "<tr><td>$dev</td><td>$fs</td><td>$type</td><td>$(humanUnits -k $size bytes )</td><td>$(humanUnits $used bytes)</td><td>$(ProgressBarH '' $(($size-$used)) $size)</td></tr>"
    done
?>
</table>
